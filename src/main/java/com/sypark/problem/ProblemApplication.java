...
public class ProblemApplication {
	...
	private Mono<String> workWithRedis(String prevHash) {
		...
		return reactiveStringRedisTemplate.opsForValue().get(keyname)
				.map(d -> new String(d))
				.switchIfEmpty(Mono.defer(() -> {
					//log.error("[redis]no data");
					return reactiveStringRedisTemplate.opsForValue().set(keyname,bigpayload,500).thenReturn(bigpayload+"first");
				}))
				.map(data -> {
					//log.error(data);
